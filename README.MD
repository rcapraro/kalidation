...
It also does _Type Checking_ on the rule: eg: an `email()` constraint is not applicable to an numeric property, so you shouldn’t be allowed to that constraint to such a property.
